...
class SSL_Dataset:
    ...
    def get_label_dset(self, is_ulb=False, strong_transform=None, onehot=False):
        """
        get_label_dset returns class BasicDataset, containing the returns of get_data.
        ...
        num_classes = self.num_classes
        crop_size = 96 if self.name.upper() == 'STL10' else 224 if self.name.upper() == 'IMAGENET' else 32
        transform = get_transform(mean[self.name], std[self.name], crop_size, False)

        return BasicDataset(self.alg, data, targets, num_classes, transform,
                            is_ulb, strong_transform, onehot)

    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
                     strong_transform=None, onehot=False, aug_path=None):
        """
        ...
        if aug_path is not None:
            aug_dataset = torchvision.datasets.ImageFolder(root=aug_path)
            aug_data = []
            for i in range(len(aug_dataset)):
                aug_data.append(aug_dataset[i][0])
            
            aug_dset = BasicDataset(self.alg, aug_data, aug_dataset.targets, self.num_classes, 
                            self.transform, False, None, onehot)
            lb_dset = ConcatDataset([lb_dset, aug_dset])
            
            print('augmentation dataset is added')
            print("len(lb_dset)", len(lb_dset))       

...
stable_diffusion_local = assets_local / 'stable-diffusion/autoencoder_kl_ema.pth'
...
def prepare():
    ...
    mox.file.copy_parallel(f's3://{args.bucket}/zebin/DPM/assets/stable-diffusion/autoencoder_kl_ema.pth', str(stable_diffusion_local))
...
if args.stage == 2:
    if args.features_path != '':
        ...
        mox.file.copy_parallel(f's3://{args.feaetures_path}', str(code_local / 'pretrained/datasets/deit_base_p4-imagenet_subsets1-2imgs_class/imagenet256_features.tar.gz'))
        ed = time.time()
    ...
    if args.generative_path != '':
        ...
        st = time.time()
        mox.file.copy_parallel(f's3://{args.generative_path}', str(code_local / 'assets/DPM/deit_base_p4-imagenet_subsets1-2imgs_class/ckpts/500000.ckpt/nnet_ema.pth'))
        ed = time.time()
        mprint(f'copying generative model cost {ed - st}s')

    cmd = f"cd {code_local} && accelerate launch {accelerate_args} train_ldm_discrete.py {script_args}"
    cmd = path_remake(cmd)
    mprint(cmd)
    os.system(cmd)

    cmd = f"cd {code_local} && accelerate launch {accelerate_args} sample_ldm_discrete_all.py {script_args} --output_path={log_local}/sample_log"
    cmd = path_remake(cmd)
    mprint(cmd)
    os.system(cmd)
